            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Welcome, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '𝐍𝐎 𝐆𝐑𝐎𝐔𝐏 𝐃𝐄𝐒𝐂𝐑𝐈𝐏𝐓𝐈𝐎𝐍') :
                            (chat.sBye || this.bye || conn.bye || 'Bye, @user!')).replace('@user', '@' + user.split`@`[0])
                        
                        this.sendButton(id, (action === 'add' ? ('*Hi,* @user') :
                            ('*Bye,* @user')).replace('@user', '@' + user.split`@`[0]), text, pp, [[action === 'add' ? '𝐖𝐄𝐋𝐂𝐎𝐌𝐄' : '𝐆𝐎𝐎𝐃𝐁𝐘𝐄', '/menu']], null, { mentions: [user] })
                    }
                }
            }
            break
